...
class Layout_Tree extends React.Component {
    ...
    convertRawToTree(nextProps) {
        ...
        diffNodeTypes.map((node) => {
            layerMatrix.push(this.linkTraversalRecurssively(linkHash, node.nodeID, 0));
            console.log("FOR NODE " + node.nodeID + " - LEVEL is: " + layerMatrix[layerMatrix.length -1]);
        });
        let numberOfLayers = (this.countUniqueNumbers(layerMatrix) * 2 + 1);
        console.log("This tree has " + numberOfLayers + " layers.");

        //label nodes with correct layer order
        //take layermatrix and scale the ranges to meet the number of layers

        //Remember to avoid carteisian product! //maybe not necessary
    }
    ...
    // *******    Check needs to happen to check if cartesian product is found.!!  **********
    linkTraversalRecurssively(linkHash, nodeID, counter) {
        console.log("counter check: " + counter);
        if (linkHash.hasOwnProperty(nodeID)) {
            let childMatrixCounter = [];
            linkHash[nodeID].map((childNode) => {
                childMatrixCounter.push(this.linkTraversalRecurssively(linkHash, childNode.target, childNode.type !== defaultNodeType ? (counter + 1) : counter));
            });
            counter = Math.max.apply(null, childMatrixCounter);
            console.log("child matrix: " , childMatrixCounter);
        }
        return counter;
    }
