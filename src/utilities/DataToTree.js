...
export function convertRawToTree(nextProps, height) {
    ...
    //create hash of nodes (speeds up search)
    ...
    //Count how many child of child extended nodes to understand maximum depth
    ...
    diffNodeTypes.map((node) => {
        ...
        counter = counter > 1 ? (counter * 2 - 1) : counter
        nodeHash[node.nodeID].layer = counter;
    });
    let totalNumOfLayers = (countUniqueNumbers(layerMatrix) * 2); 
    
    //scale for vertical placement
    let scaleHeight = d3.scaleLinear().domain([totalNumOfLayers,0]).range([40,height-40]); 
    ...
    nextProps.state.links.map((link) => {
        if(nodeHash[link.source].type !== defaultNodeType){
            if(!nodeHash[link.target].hasOwnProperty("layerArray")){
                nodeHash[link.target].layerArray = [];
            }
            let layer = nodeHash[link.source].layer - 1;
            nodeHash[link.target].layer = layer;
            nodeHash[link.target].scaledLayer = scaleHeight(layer);
            nodeHash[link.target].layerArray.push(layer);   
        }else{
            if(!nodeHash[link.source].hasOwnProperty("layerArray")){
                nodeHash[link.source].layerArray = [];
            }
            let layer = nodeHash[link.target].layer + 1
            nodeHash[link.source].layer = layer;
            nodeHash[link.source].scaledLayer = scaleHeight(layer);
            nodeHash[link.source].layerArray.push(layer);   
        }
